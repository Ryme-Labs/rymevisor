...
func (r *NetworkRepository) GetByID(ctx context.Context, id string) (*domain.PrivateNetwork, error) {
	...
	err := r.pool.QueryRow(ctx,
		...
		&net.ID, &net.Name, &net.OrganizationID, &net.VpcID, &net.Type, &net.CIDR, &net.IPv6CIDR,
...
func (r *NetworkRepository) List(ctx context.Context, filter domain.NetworkFilter) ([]*domain.PrivateNetwork, int, error) {
	var total int
	err := r.pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM private_networks WHERE organization_id = $1`, filter.OrganizationID,
	).Scan(&total)
	if err != nil {
		return nil, 0, err
	}
	...
	rows, err := r.pool.Query(ctx,
		`SELECT id, name, organization_id, vpc_id, type, cidr::text, ipv6_cidr::text, internet_gateway, labels, created_at, updated_at
		 FROM private_networks WHERE organization_id = $1
		 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
		filter.OrganizationID, filter.PerPage, offset,
	)
	if err != nil {
		return nil, 0, err
	}
